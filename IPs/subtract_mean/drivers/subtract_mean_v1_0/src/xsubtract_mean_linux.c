...
typedef struct {
    ...
    XSubtract_mean_uio_map maps[ MAX_UIO_MAPS ];
} XSubtract_mean_uio_info;

/***************** Variable Definitions **************************************/
static XSubtract_mean_uio_info uio_info;
...
static int uio_info_read_name(XSubtract_mean_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSubtract_mean_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSubtract_mean_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSubtract_mean_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSubtract_mean_Initialize(XSubtract_mean *InstancePtr, const char* InstanceName) {
	XSubtract_mean_uio_info *InfoPtr = &uio_info;
